...
def checkPos(u, v, datum, visited):
    if u < 0 or u >= DIGIT_DATUM_HEIGHT or v < 0 or v >= DIGIT_DATUM_WIDTH:
        return False
    if datum.getPixel(v, u) != 0:
        return False
    if visited[(u, v)] == 1:
        return False
    return True
    ...
    for i in range(height):
        for j in range(width):
            ...
            while queue.isEmpty() == False:
                u, v = queue.pop()
                if checkPos(u - 1, v, datum, visited):
                    queue.push((u - 1, v))
                    visited[(u - 1, v)] = 1
                if checkPos(u + 1, v, datum, visited):
                    queue.push((u + 1, v))
                    visited[(u + 1, v)] = 1
                if checkPos(u, v - 1, datum, visited):
                    queue.push((u, v - 1))
                    visited[(u, v - 1)] = 1
                if checkPos(u, v + 1, datum, visited):
                    queue.push((u, v + 1))
                    visited[(u, v + 1)] = 1
    for i in range(-20, 0):
        features[i] = (numComponents > 1)
    for i in range(-40, -20):
        features[i] = (numComponents > 3)
    for i in range(-60, -40):
        features[i] = (numComponents > 5)
    for i in range(-70, -60):
        features[i] = (float(numUps) / nonZeros * 100.0 >= 50.0)
    # for i in range(-40, -30):
    #     features[i] = (numComponents >= 4)
    print datum, float(numUps) / nonZeros * 100.0
    # for i in range(-40, -30):
    #     features[i] = (nonZeros > 100)
    # for i in range(-300, -200):
    #     features[i] = (nonZeros > 150)
    # print numComponents, nonZeros
...
def enhancedPacmanFeatures(state, action):
    ...
    pacman = next_state.getPacmanPosition()
    foods = next_state.getFood().asList()
    capsules = next_state.getCapsules()
    ghosts = next_state.getGhostPositions()

    ghost_dis = [util.manhattanDistance(pacman, x) for x in ghosts]
    ghost_dis = sorted(ghost_dis)
    if len(ghost_dis) > 0:
        features['ghost'] = 3.0 / (1.0 + ghost_dis[0])

    capsule_dis = [util.manhattanDistance(pacman, x) for x in capsules]
    capsule_dis = sorted(capsule_dis)
    if len(capsule_dis) > 0:
        features['capsule'] = 1.0 / (1.0 + capsule_dis[0])

    food_dis = [util.manhattanDistance(pacman, x) for x in foods]
    food_dis = sorted(food_dis)
    for i in range(min(len(food_dis), 3)):
        features[('food', i)] = 2.0 / (1.0 + food_dis[i])

    # features["capsule count"] = (5.0 / (1.0 + len(capsules)))
    # features["score"] = 5.0 / (1.0 + state.getScore())
